...
export const ColumnWidth: TextFieldClientComponent = (props: TextFieldClientProps) => {
  ...
  const columnWidths =
    siblingData.columns && siblingData.columns.map((column: Column) => column.columnWidth)
  ...
  const results =
    ...
    columnWidths.map((width: string) => {
      // we'll do this by mapping through our columnWidths
      if (width?.includes('auto')) {
        // and first check if width includes 'auto'
  ...
  const percentage =
    results && isNaN(Math.ceil(results.reduce((acc: number, column: number) => acc + column, 0)))
      ? '100%'
  ...
  useEffect(() => {
    // so create a useEffect hook and pass in the arrow function
