...
export const Row: Block = {
  // export a constant called Row with the Block type assigned to it
  ...
  fields: [
    // and will have two fields
    {
      ...
      validate: (value: any) => {
        // and will validate that the value is equal to 100%
        if (value === '100%') {
      ...
      admin: {
        // we'll use the admin property to make the field read-only and to override the default Field component
        readOnly: true, // set readOnly to true
        components: {
          // we'll come back to this
          Field: {
            // then to wrap everything up, we can import our ColumnWidth component here
            path: 'src/collections/fields/ColumnWidth.tsx',
          },
          Error: {
            // we can pull in our custom error component we edited earlier
            path: 'src/components/Admin/Fields/Error.tsx#Error',
            clientProps: {
              // and using the clientProps prop
